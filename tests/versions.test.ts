...
import { BaseVersions, QuickTVVersions } from '../src/versions';
...
describe('ElectronVersions', () => {
  ...
  describe('.create', () => {
    it('does not fetch with a fresh cache', async () => {
      ...
      const { versions } = await QuickTVVersions.create({ versionsCache });
    ...
    it('throws an error with a missing cache and failed fetch', async () => {
      ...
      await fs.remove(versionsCache);
      await expect(QuickTVVersions.create({ versionsCache })).rejects.toThrow(
        Error,
      );
      expect(scope.isDone());
    });
  ...
  describe('.fetch', () => {
    it('updates the cache', async () => {
      const electronVersions = await QuickTVVersions.create({ versionsCache });
